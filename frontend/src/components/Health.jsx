  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-xs flex flex-col h-full">
      ...
      <div className="flex items-center justify-between pb-4 border-b border-slate-100">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">Medicines, habits & SMS alerts</p>
          </div>
        </div>
      ...
      <div className="mt-4 p-3 bg-slate-50 rounded-xl flex items-center justify-between">
        <div>
          <span className="text-xs font-medium text-slate-500 uppercase tracking-wider block">Active Habits</span>
          <span className="text-xl font-extrabold text-slate-900">{summary.totalCount}</span>
        </div>
        <div className="text-right">
          <span className="text-xs font-medium text-emerald-600 block flex items-center gap-1 justify-end">
            <Repeat className="w-3 h-3" /> {summary.dailyCount} Daily
          </span>
          {summary.smsAlertsEnabledCount > 0 && (
            <span className="text-[11px] text-slate-500 font-medium flex items-center gap-0.5 justify-end mt-0.5">
              <Smartphone className="w-3 h-3 text-sky-500" /> {summary.smsAlertsEnabledCount} SMS active
            </span>
          )}
        </div>
      </div>
      ...
      {isAdding && (
        <form onSubmit={handleCreateSubmit} className="mt-4 p-4 bg-slate-50 rounded-xl border border-slate-200 space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-700">Add Health Habit</span>
          ...
          <div>
            <div className="relative">
              <input
                ...
                className="w-full text-xs px-3 py-2 pl-8 bg-white border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500"
              />
      ...
      <div className="mt-4 flex-1 overflow-y-auto space-y-2.5 max-h-[420px] pr-0.5">
        {isLoading ? (
          ...
          <div className="text-center py-8 text-slate-400">
            ...
            <p className="text-xs font-medium">No health reminders added.</p>
            <p className="text-[11px] text-slate-400 mt-0.5">Schedule daily water, vitamins, or walking habits.</p>
          </div>
        ) : (
          reminders.map((reminder) => (
            <div
              key={reminder.id}
              className="p-3.5 rounded-xl border border-slate-200 bg-white hover:border-slate-300 transition-all flex items-center justify-between gap-3"
            >
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <h4 className="text-xs font-bold text-slate-900 truncate">{reminder.name}</h4>
                  <div className="flex items-center gap-2 mt-0.5 text-[11px] text-slate-500">
                    ...
                    {reminder.phoneNumber && (
                      <>
                        <span>•</span>
                        <span className="text-[10px] text-sky-600 font-medium flex items-center gap-0.5" title={`SMS active to ${reminder.phoneNumber}`}>
